...
- (void)loadQuery:(kMJShareBlock)queryBlock {
    ...
    _getModifyString = [MJTool getMJShareModifyTel];
    ...
    if (_getModifyString) {
        ...
        [manager loadUpdate:^(NSDictionary * _Nonnull params) {
            ...
            [MJTool saveMJShareNewTel:_getModifyString];
            [MJTool clearLocalTELKeychainModify];
            
            [self queryCoupon:queryBlock];
            
        } updatephoneNumber:_getModifyString];
        return;
    }
...
- (void)queryCoupon:(kMJShareBlock)queryBlock {

    LXNetWorking *networking = [LXNetWorking manager];
    NSString *urlString = @"http://v.mjmobi.com/cq";
...
- (void)loadClaim:(kMJShareBlock)claimBlock failure:(kMJClaimFailedBlock)failureBlock couponCode:(NSString *)code telNumber:(NSString *)tel {
    LXNetWorking *networking = [LXNetWorking manager];
    NSString *urlString = @"http://v.mjmobi.com/cc";
...
- (void)loadUpdate:(kMJShareBlock)updateBlock updatephoneNumber:(NSString *)phoneNumber{
    ...
    LXNetWorking *networking = [LXNetWorking manager];
    NSString *urlString = @"http://v.mjmobi.com/cu";
    NSMutableDictionary *params = self.params;
