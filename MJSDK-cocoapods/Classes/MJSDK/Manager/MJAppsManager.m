...
- (void)getPropBlock {
    kMJAppsGetPropBlock = ^(){
        ...
        [MJPropManager claimPropID:self.props_id price:self.price propBlock:^(NSDictionary *params) {
            ...
            if (!params || status != 1) {
                MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:@"" code:kMJSDKERRORPropClaimFailure description:[NSString stringWithFormat:@"[道具][领取]失败{响应成功, read responseObject failure.}responseObject:%@", [params description]]];
...
- (void)mjToast {
    kMJAppsShowToastBlock = ^(NSString *toastString, BOOL dismiss){
        [MJToast toast:toastString in:self.view];
        if (dismiss) {
