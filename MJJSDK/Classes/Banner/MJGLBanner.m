...
- (instancetype)init{
    if (self = [super init]) {
        [self setUp];
//        [self setMjBannerPosition:KMJBannerPositionTop];
    }
    return self;
}
...
- (void)fill:(NSDictionary *)params{
    ...
    NSString *detail = [NSString stringWithFormat:@"Detail %d", index];
    NSString *btnTitle = [NSString stringWithFormat:@"Btn %d", index];
//    [self.imgView_logo setImage:[UIImage imageNamed:@""]];
    [self.lab_title setText:title];
    [self.lab_detail setText:detail];
//    [self.btn_dl setTitle:btnTitle forState:UIControlStateNormal];
}
